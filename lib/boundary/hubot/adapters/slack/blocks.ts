...
const howToTestLead = ({ runbookRootUrl, runbookTestLeadUrl }: AppConfig) => {
  const url = runbookTestLeadUrl ?? runbookRootUrl;
  return `:${testLead()}: <${url}|How to run test lead>`;
};
...
export const helpBlocks = (config: AppConfig) => {
  return [
    ...
    mrkdownBlock(howToTestLead(config)),
