...
export class Slack extends Adapter implements CommPlatform {
  ...
  sendPointNag(incident: Incident, mainRoom: string): Promise<unknown> {
    ...
    const mainRoomAlert = `> :${siren()}: *No .point set for:* ${
      incident.title
  ...
  sendCommsNag(incident: Incident, mainRoom: string): Promise<unknown> {
    ...
    const mainRoomAlert = `> :${siren()}: *No .comms set for:* ${
      incident.title
  ...
  updateBreakingTopic(incident: Incident, tracker?: IssueTracker) {
    ...
    const { priority, title, point, comms, triage, engLead, testLead } =
      incident;

    const priorityLabel = this.#priorityLabel(priority);
    const trackerLink = this.#trackerLink(incident, tracker);

    const pointStr = point ? `Point: <@${point}>` : "";
    const commsStr = comms ? `Comms: <@${comms}>` : "";
    const triageStr = triage ? `Triage: <@${triage}>` : "";
    const engLeadStr = engLead ? `Eng: <@${engLead}>` : "";
    const testLeadStr = testLead ? `Test: <@${testLead}>` : "";

    const topic = [
      `:rotating_light: ${priorityLabel}${trackerLink} ${title}`,
      pointStr,
      commsStr,
      triageStr,
      engLeadStr,
    ...
    return this.#setTopic(incident.chatRoomUid, topic);
  }
  ...
  sendTestLeadTakeover(
    ...
    return this.#replyThreaded(
      channel,
      [
        headerBlock("Test Lead Takeover"),
        mrkdownBlock(`Congrats <@${testLead}>, you are test lead!`),
        mrkdownBlock(`<${runbookUrl}|Breaking Incident Runbook> :closed_book:`),
        mrkdownBlock(
          "Please coordinate testing efforts and verify fixes. Thanks!"
        ),
      ],
      `Congrats <@${testLead}>, you are test lead!`,
      timestamp
    );
  }
  ...
  #trackerLink(incident: Incident, tracker?: IssueTracker): string {
    if (!tracker || !incident.trackerUid) {
      return "";
    }
    return ` | <${tracker.fmtUidForSlack(incident.trackerUid)}|${
      incident.trackerUid
    }>`;
  }
  ...
  async #setTopic(channel: string, topic: string): Promise<void> {
    try {
      await this.#webClient.conversations.setTopic({
        channel,
        topic: topic.substring(0, MAX_CHANNEL_TOPIC_LENGTH),
      });
    } catch (error) {
      this.robot.logger.error(`Failed to set topic in ${channel}: ${error}`);
    }
  }
}
