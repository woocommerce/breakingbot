...
export const incidentSetTestLead = async (
  robot: BreakingBot,
  room: string,
  testLead: string,
  createdBy: string,
  ...
  if (testLead === incident.testLead) {
    return robot.adapter.reactToMessage(room, "ok_hand", messageId);
  }

  const entry = userCacheGet(robot.users, testLead);
  const isValid = entry ? true : await robot.adapter.validateUser(testLead);

  if (!isValid) {
    return robot.adapter.reactToMessage(room, "exclamation", messageId);
  }

  const { runbookTestLeadUrl, runbookRootUrl } = robot.config;
  const runbookUrl = runbookTestLeadUrl ?? runbookRootUrl;

  const [result] = await setIncidentTestLeadDb(robot.db, incident.id, testLead);

  if (result?.testLead !== testLead) {
    return robot.adapter.sendError(room, "Failed to set testlead!", messageId);
  }

  incident.testLead = testLead;

  const url = await permalink(robot.adapter, room, messageId);
  const tasks = [
    robot.adapter.sendTestLeadTakeover(room, testLead, runbookUrl, messageId),
    robot.adapter.updateBreakingTopic(incident, robot.tracker),
    addLogEventDb(
      robot.db,
      incident.id,
      `${testLead} is now on test`,
      createdBy,
      url
    ),
  ];
