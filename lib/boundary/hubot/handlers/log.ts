...
export const logRemoveFactor = async (
  robot: BreakingBot,
  room: string,
  factorText: string,
  messageId?: string
  ...
  const removed = await removeLogContributingFactorDb(
    robot.db,
    incident.id,
    factorText
  );
  ...
  if (!removed) {
    const errMsg = `Unable to remove factor \`${factorText}\`. Maybe see \`.factors\`?`;
